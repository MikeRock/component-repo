...
export default class RippleButton extends Component {
  ...
  componentDidMount() {
    if ("ontouchstart" in window) this.el.addEventListener("touchstart", this.onTouch);
    else this.el.addEventListener("mousedown", this.onTouch);
  }
  ...
  render() {
    return (
      <div ref={el => (this.el = el)} className={_["button__wrapper"]}>
        ...
        <div ref={el => (this.ripple = el)} className={_["button__ripple"]} />
      </div>
    );
  }
}
