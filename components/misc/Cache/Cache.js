...
export default class Cache extends Component {
  ...
  run() {
    ...
    const promise = this.props.manifest
      ? new Promise((resolve, reject) => {
          resolve(btoa(JSON.stringify(this.props.manifest)));
        })
      : fetch(this.props.api, config)
          .then(resp => resp.json())
          .then(manifest => {
    ...
    return promise.then(param =>
      window.navigator.serviceWorker
        .getRegistration(`./worker.js?param=${param}${this.props.version ? `&version=${this.props.version}` : ""}`)
        .then(registration => {
          if (!registration) {
            return navigator.serviceWorker
              .register(`./worker.js?param=${param}${this.props.version ? `&version=${this.props.version}` : ""}`, { scope: "/" })
              .then(registration => {
